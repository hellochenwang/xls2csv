...
static const char *encoding_for_codepage(WORD codepage) {
  ...
  struct codepage_entry_t *result =
      bsearch(&key, _codepage_entries,
...
static char *unicode_decode_iconv(const char *s, size_t len, iconv_t ic) {
  ...
  if (s && len && ic) {
    ...
    if (outbuf) {
      out_ptr = outbuf;
      while (inlenleft) {
        st = iconv(ic, (ICONV_CONST char **)&src_ptr, &inlenleft,
...
char *codepage_decode(const char *s, size_t len, xlsWorkBook *pWB) {
  ...
  if (!pWB->converter) {
    const char *from_encoding =
        pWB->is5ver ? encoding_for_codepage(pWB->codepage) : "ISO-8859-1";
    iconv_t converter = iconv_open(pWB->charset, from_encoding);
    if (converter == (iconv_t)-1) {
      printf("conversion from '%s' to '%s' not available", from_encoding,
             pWB->charset);
      return NULL;
    }
...
void xls_showBOF(BOF *bof) {
  printf("----------------------------------------------\n");
  verbose("BOF");
  printf("   ID: %.4Xh %s (%s)\n", bof->id, brdb[get_brbdnum(bof->id)].name,
         brdb[get_brbdnum(bof->id)].desc);
  printf("   Size: %i\n", bof->size);
}
...
void xls_showColinfo(struct st_colinfo_data *col) {
  ...
  if (col->flags & 0x1) printf("hidden ");
  if (col->flags & 0x700) printf("outline ");
  if (col->flags & 0x1000) printf("collapsed ");
  printf(")\n");
  printf("----------------------------------------------\n");
}
...
void xls_showCell(struct st_cell_data *cell) {
  printf("  -----------\n");
  printf("     ID: %.4Xh %s (%s)\n", cell->id, brdb[get_brbdnum(cell->id)].name,
         brdb[get_brbdnum(cell->id)].desc);
  printf("   Cell: %c:%u  [%u:%u]\n", cell->col + 'A', cell->row + 1, cell->col,
         cell->row);
...
char *xls_getfcell(xlsWorkBook *pWB, struct st_cell_data *cell, BYTE *label) {
  ...
  if (cell->xf < pWB->xfs.count) xf = &pWB->xfs.xf[cell->xf];
...
char *xls_getCSS(xlsWorkBook *pWB) {
  ...
  for (i = 0; i < pWB->xfs.count; i++) {
    ...
    if (xf->font)
      sprintf(color, "color:#%.6X;",
              xls_getColor(pWB->fonts.font[xf->font - 1].color, 0));
    else
      sprintf(color, "%s", "");
    ...
    sprintf(buf,
            ".xf%i{ font-size:%ipt;font-family: "
            "\"%s\";background:#%.6X;text-align:%s;vertical-align:%s;%s%s%s%s%"
            "s%s%s%s}\n",
            i, size, fontname, background, align, valign, borderleft,
            borderright, bordertop, borderbottom, color, italic, bold,
            underline);
