...
static ssize_t sector_read(OLE2 *ole2, void *buffer, size_t buffer_len,
                           DWORD sid);
...
static int ole2_validate_sector_chain(DWORD *chain, DWORD chain_count,
                                      DWORD chain_start) {
  DWORD count = 0;
  DWORD sector = chain_start;
  while (sector != ENDOFCHAIN) {
    if (sector >= chain_count) return 0;
...
static int ole2_validate_sector(DWORD sector, OLE2 *ole) {
  if (sector >= ole->SecIDCount) {
    if (xls_debug)
      fprintf(stderr, "Error: fatpos %d out-of-bounds for SecID[%d]\n",
  ...
  if (sector == xlsIntVal(ole->SecID[sector])) {
    if (xls_debug)
      fprintf(stderr, "Error: Sector loop detected, SecID[%d] = %d\n",
...
static int ole2_bufread(OLE2Stream *olest) {
  ...
  if (olest == NULL || olest->ole == NULL) return -1;

  if ((DWORD)olest->fatpos != ENDOFCHAIN) {
    if (olest->sfat) {
      if (olest->ole->SSAT == NULL || olest->buf == NULL ||
          olest->ole->SSecID == NULL)
        return -1;

      if (olest->fatpos * olest->ole->lssector + olest->bufsize >
          olest->ole->SSATCount) {
        if (xls_debug)
          fprintf(stderr, "Error: fatpos %d out-of-bounds for SSAT\n",
                  (int)olest->fatpos);
        return -1;
      }
      ...
      if (olest->fatpos >= olest->ole->SSecIDCount) {
        if (xls_debug)
          fprintf(stderr, "Error: fatpos %d out-of-bounds for SSecID[%d]\n",
      ...
      if ((int)olest->fatpos < 0 ||
          sector_read(olest->ole, olest->buf, olest->bufsize, olest->fatpos) ==
              -1) {
        if (xls_debug)
...
ssize_t ole2_read(void *buf, size_t size, size_t count, OLE2Stream *olest) {
  ...
  if ((long)olest->size >= 0 && !olest->sfat)  // directory is -1
  {
    ...
    // (olest->cfat*olest->ole->lsector+olest->pos) );
  ...
  while ((!olest->eof) && (didReadCount < totalReadCount)) {
    ...
    if (needToReadCount < remainingBytes) {  // does the current sector contain
                                             // all the data I need?
      memcpy((BYTE *)buf + didReadCount, olest->buf + olest->pos,
             needToReadCount);
      ...
      if (ole2_bufread(olest) == -1) return -1;
    }
    if (((DWORD)olest->fatpos == ENDOFCHAIN) &&
        (olest->pos >= olest->bufsize)) {
      olest->eof = 1;
    }
  }
  if (didReadCount > totalReadCount) return -1;
  ...
  if (olest->sfat) {
    ...
    if (div_rez.quot != 0) {
      for (i = 0; i < div_rez.quot; i++) {
        if (olest->fatpos >= olest->ole->SSecIDCount) return -1;
        olest->fatpos = xlsIntVal(olest->ole->SSecID[olest->fatpos]);
      }
    }
    ...
    if (div_rez.quot != 0) {
      for (i = 0; i < div_rez.quot; i++) {
        if (!ole2_validate_sector(olest->fatpos, olest->ole)) return -1;
        olest->fatpos = xlsIntVal(olest->ole->SecID[olest->fatpos]);
      }
    }
...
#endif
    if (str && strcmp(str, file) ==
                   0)  // newer versions of Excel don't write the "Root Entry"
                       // string for the first set of data
    {
...
static int ole2_fseek(OLE2 *ole2, size_t pos) {
  if (ole2->file) return fseek(ole2->file, pos, SEEK_SET);

  if (pos > ole2->buffer_len) return -1;
...
static size_t ole2_fread(OLE2 *ole2, void *buffer, size_t buffer_len,
                         size_t size) {
  if (size > buffer_len) return 0;

  memset(buffer, 0, size);

  if (ole2->file) return fread(buffer, 1, size, ole2->file) > 0;

  if (ole2->buffer_pos >= ole2->buffer_len) return 0;
...
static ssize_t ole2_read_header(OLE2 *ole) {
  ...
  // make sure the file looks good. Note: this code only works on Little Endian
  // machines
  if (oleh->id[0] != 0xE011CFD0 || oleh->id[1] != 0xE11AB1A1 ||
...
static ssize_t ole2_read_body(OLE2 *ole) {
  ...
  do {
    ...
    if (pss->type == PS_USER_ROOT ||
        pss->type == PS_USER_STREAM)  // (name!=NULL) //
    {
#ifdef OLE_DEBUG
      ...
      if (pss->sstart == ENDOFCHAIN) {
        ...
        blocks =
            (pss->size + (ole->lsector - 1)) / ole->lsector;  // count partial
#ifdef OLE_DEBUG
        fprintf(stderr, "OLE BLOCKS: %d = (%d + (%d - 1))/%d\n", (int)blocks,
                (int)pss->size, (int)ole->lsector, (int)ole->lsector);
#endif
        if ((ole->SSAT = ole_realloc(ole->SSAT, blocks * ole->lsector)) ==
            NULL) {
          total_bytes_read = -1;
          goto cleanup;
        }
        ...
        for (k = 0; k < blocks; ++k) {
          ...
          if (sector == ENDOFCHAIN ||
              sector_read(ole, wptr, bytes_left, sector) == -1) {
            if (xls_debug)
...
static ssize_t sector_read(OLE2 *ole2, void *buffer, size_t buffer_len,
  ...
  if ((seeked = ole2_fseek(ole2, sector_pos(ole2, sid))) != 0) {
    if (xls_debug)
      fprintf(stderr, "Error: wanted to seek to sector %u (0x%x) loc=%u\n", sid,
              sid, (unsigned int)sector_pos(ole2, sid));
    return -1;
  }

  if ((num = ole2_fread(ole2, buffer, buffer_len, ole2->lsector)) != 1) {
    if (xls_debug)
      fprintf(stderr, "Error: fread wanted 1 got %lu loc=%u\n",
              (unsigned long)num, (unsigned int)sector_pos(ole2, sid));
    return -1;
  }
...
static ssize_t read_MSAT_header(OLE2 *ole2, OLE2Header *oleh,
                                DWORD sectorCount) {
  ...
  for (sectorNum = 0; sectorNum < sectorCount && sectorNum < 109; sectorNum++) {
    if ((bytes_read = sector_read(ole2, sector, bytes_left,
                                  oleh->MSAT[sectorNum])) == -1) {
      if (xls_debug)
        fprintf(stderr, "Error: Unable to read sector #%d\n",
                oleh->MSAT[sectorNum]);
      return -1;
...
static ssize_t read_MSAT_body(OLE2 *ole2, DWORD sectorOffset,
                              DWORD sectorCount) {
  ...
  while (sid != ENDOFCHAIN &&
         sid != FREESECT)  // FREESECT only here due to an actual file that
                           // requires it (old Apple Numbers bug)
  {
    int posInSector;
    // read MSAT sector
    ...
    // read content
    for (posInSector = 0; posInSector < (ole2->lsector - 4) / 4;
         posInSector++) {
      DWORD s = sector[posInSector];
      // printf("   s[%d]=%d (0x%x)\n", posInSector, s, s);

      if (s != ENDOFCHAIN &&
          s != FREESECT)  // see patch in Bug 31. For very large files
      {
        ...
        if ((bytes_read = sector_read(
                 ole2, (BYTE *)(ole2->SecID) + sectorNum * ole2->lsector,
                 (ole2->SecIDCount * sizeof(DWORD) - sectorNum * ole2->lsector),
                 s)) == -1) {
          if (xls_debug)
            fprintf(stderr, "Error: Unable to read sector #%d\n", s);
          total_bytes_read = -1;
...
static ssize_t read_MSAT_trailer(OLE2 *ole2) {
  ...
  if (ole2->sfatstart == ENDOFCHAIN) return 0;

  if ((ole2->SSecID = ole_malloc(ole2->csfat * (size_t)ole2->lsector)) ==
      NULL) {
    return -1;
  }
  ...
  for (k = 0; k < ole2->csfat; ++k) {
    if (sector == ENDOFCHAIN ||
        sector_read(ole2, wptr, bytes_left, sector) == -1) {
      total_bytes_read = -1;
      goto cleanup;
    }
...
static ssize_t read_MSAT(OLE2 *ole2, OLE2Header *oleh) {
  ...
  if (count == 0 || count > (1 << 24)) {
    if (xls_debug)
      fprintf(stderr, "Error: MSAT count %u out-of-bounds\n", count);
    return -1;
  }
  ...
  if ((bytes_read = read_MSAT_body(ole2, total_bytes_read / ole2->lsector,
                                   count)) == -1) {
    total_bytes_read = -1;
    goto cleanup;
  }
