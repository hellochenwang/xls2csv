...
static xls_error_t xls_addSST(xlsWorkBook* pWB, SST* sst, DWORD size) {
  ...
  if (sst->num > (1 << 24)) return LIBXLS_ERROR_MALLOC;

  if (pWB->sst.string) return LIBXLS_ERROR_PARSE;
...
static xls_error_t xls_appendSST(xlsWorkBook* pWB, BYTE* buf, DWORD size) {
  ...
  while (ofs < size) {
    ...
    if (!pWB->sst.continued || (pWB->sst.continued && ln != 0)) {
      ...
      if (flag & 0x4) {
        ...
        sz = buf[ofs + 0] + (buf[ofs + 1] << 8) + (buf[ofs + 2] << 16) +
             ((DWORD)buf[ofs + 3] << 24);
        ofs += 4;
    ...
    if (ln_toread > 0 || !pWB->sst.continued) {
      ...
      if (xls_debug) {
        printf("String %4u: %s<end>\n", pWB->sst.lastid - 1,
               pWB->sst.string[pWB->sst.lastid - 1].str);
      }
    } else {
      free(ret);
    }
  ...
  // Save current character count and count of rich text formatting runs and
  // size of asian phonetic settings block
  if (ln > 0 || rt > 0 || sz > 0) {
    pWB->sst.continued = 1;
    pWB->sst.lastln = ln;
...
static xls_error_t xls_addSheet(xlsWorkBook* pWB, BOUNDSHEET* bs, DWORD size) {
  ...
  pWB->sheets.sheet =
      realloc(pWB->sheets.sheet,
              (pWB->sheets.count + 1) * sizeof(struct st_sheet_data));
  if (pWB->sheets.sheet == NULL) return LIBXLS_ERROR_MALLOC;
...
static xls_error_t xls_addRow(xlsWorkSheet* pWS, ROW* row) {
  ...
  if (row->index > pWS->rows.lastrow) return LIBXLS_ERROR_PARSE;
...
static xls_error_t xls_makeTable(xlsWorkSheet* pWS) {
  ...
  if ((pWS->rows.row =
           calloc((pWS->rows.lastrow + 1), sizeof(struct st_row_data))) == NULL)
    return LIBXLS_ERROR_MALLOC;
  ...
  for (t = 0; t <= pWS->rows.lastrow; t++) {
    ...
    if ((tmp->cells.cell =
             calloc(tmp->cells.count, sizeof(struct st_cell_data))) == NULL)
      return LIBXLS_ERROR_MALLOC;
...
int xls_isCellTooSmall(xlsWorkBook* pWB, BOF* bof, BYTE* buf) {
  if (bof->size < sizeof(COL)) return 1;

  if (bof->id == XLS_RECORD_FORMULA || bof->id == XLS_RECORD_FORMULA_ALT)
    return (bof->size < sizeof(FORMULA));

  if (bof->id == XLS_RECORD_MULRK) return (bof->size < offsetof(MULRK, rk));
  ...
  if (bof->id == XLS_RECORD_LABEL || bof->id == XLS_RECORD_RSTRING) {
    if (bof->size < offsetof(LABEL, value) + 2) return 1;
    ...
    if (bof->size < offsetof(LABEL, value) + 3) return 1;
  ...
  if (bof->id == XLS_RECORD_RK) return (bof->size < sizeof(RK));

  if (bof->id == XLS_RECORD_NUMBER) return (bof->size < sizeof(BR_NUMBER));

  if (bof->id == XLS_RECORD_BOOLERR) return (bof->size < sizeof(BOOLERR));
...
static struct st_cell_data* xls_addCell(xlsWorkSheet* pWS, BOF* bof,
                                        BYTE* buf) {
  ...
  if (xls_isCellTooSmall(pWS->workbook, bof, buf)) return NULL;

  // printf("ROW: %u COL: %u\n", xlsShortVal(((COL*)buf)->row),
  // xlsShortVal(((COL*)buf)->col));
  ...
  switch (bof->id) {
    ...
    case XLS_RECORD_FORMULA_ALT:
      ...
      if (((FORMULA*)buf)->res != 0xffff) {
        ...
        cell->l = 0;
        memcpy(&cell->d, &((FORMULA*)buf)->resid,
               sizeof(double));        // Required for ARM
    ...
    case XLS_RECORD_MULRK:
      for (i = 0; i < (bof->size - 6) / 6;
           i++)  // 6 == 2 row + 2 col + 2 trailing index
      {
        WORD index = col + i;
        if (index >= row->cells.count) {
          if (xls_debug)
            fprintf(stderr, "Error: MULTI-RK index out of bounds\n");
          return NULL;
        }
    ...
    case XLS_RECORD_RSTRING:
      xls_cell_set_str(cell,
                       xls_getfcell(pWS->workbook, cell, ((LABEL*)buf)->value));
      if (cell->str) {
    ...
    case XLS_RECORD_NUMBER:
      ...
      memcpy(&cell->d, &((BR_NUMBER*)buf)->value,
             sizeof(double));  // Required for ARM
      xls_cell_set_str(cell, xls_getfcell(pWS->workbook, cell, NULL));
      break;
...
static char* xls_addFont(xlsWorkBook* pWB, FONT* font, DWORD size) {
  ...
  pWB->fonts.font = realloc(
      pWB->fonts.font, (pWB->fonts.count + 1) * sizeof(struct st_font_data));
  if (pWB->fonts.font == NULL) return NULL;
...
static xls_error_t xls_addFormat(xlsWorkBook* pWB, FORMAT* format, DWORD size) {
  ...
  pWB->formats.format =
      realloc(pWB->formats.format,
              (pWB->formats.count + 1) * sizeof(struct st_format_data));
  if (pWB->formats.format == NULL) return LIBXLS_ERROR_MALLOC;

  tmp = &pWB->formats.format[pWB->formats.count];
  tmp->index = format->index;
  tmp->value = get_string(format->value, size - offsetof(FORMAT, value),
                          (BYTE)!pWB->is5ver, pWB);
...
static xls_error_t xls_addXF8(xlsWorkBook* pWB, XF8* xf) {
  ...
  verbose("xls_addXF");
  pWB->xfs.xf =
      realloc(pWB->xfs.xf, (pWB->xfs.count + 1) * sizeof(struct st_xf_data));
  if (pWB->xfs.xf == NULL) return LIBXLS_ERROR_MALLOC;
...
static xls_error_t xls_addColinfo(xlsWorkSheet* pWS, COLINFO* colinfo) {
  ...
  pWS->colinfo.col =
      realloc(pWS->colinfo.col,
              (pWS->colinfo.count + 1) * sizeof(struct st_colinfo_data));
  if (pWS->colinfo.col == NULL) return LIBXLS_ERROR_MALLOC;
...
static xls_error_t xls_mergedCells(xlsWorkSheet* pWS, BOF* bof, BYTE* buf) {
  if (bof->size < sizeof(WORD)) return LIBXLS_ERROR_PARSE;
  ...
  for (i = 0; i < count; i++) {
    ...
    // Sanity check:
    if (!(span->rowf <= span->rowl && span->rowl <= pWS->rows.lastrow &&
          span->colf <= span->coll && span->coll <= pWS->rows.lastcol)) {
      return LIBXLS_ERROR_PARSE;
    }
    ...
    pWS->rows.row[span->rowf].cells.cell[span->colf].colspan =
        (span->coll - span->colf + 1);
    pWS->rows.row[span->rowf].cells.cell[span->colf].rowspan =
        (span->rowl - span->rowf + 1);
  ...
  do {
    if (xls_debug > 10) {
      ...
      printf("  OLE: start=%d pos=%u size=%u fatPos=%u\n", pWB->olestr->start,
             (unsigned int)pWB->olestr->pos, (unsigned int)pWB->olestr->size,
             (unsigned int)pWB->olestr->fatpos);
    }
    ...
    switch (bof1.id) {
      ...
      case XLS_RECORD_FORMAT:
        ...
        if ((retval = xls_addFormat(pWB, (FORMAT*)buf, bof1.size)) !=
            LIBXLS_OK) {
          goto cleanup;
        }
        break;
      ...
      case XLS_RECORD_1904:
        pWB->is1904 = *(BYTE*)buf;  // the field is a short, but with little
                                    // endian the first byte is 0 or 1
        if (xls_debug) {
          printf("   mode: 0x%x\n", pWB->is1904);
        }
        break;
      ...
      default:
        if (xls_debug) {
          // xls_showBOF(&bof1);
          printf(
              "    Not Processed in parseWoorkBook():  BOF=0x%4.4X size=%d\n",
              bof1.id, bof1.size);
        }
        break;
    }
...
static xls_error_t xls_preparseWorkSheet(xlsWorkSheet* pWS) {
  ...
  do {
    ...
    if (tmp.size) {
      ...
      if ((read = ole2_read(buf, 1, tmp.size, pWS->workbook->olestr)) !=
          tmp.size) {
        if (xls_debug) fprintf(stderr, "Error: failed to read OLE block\n");
        retval = LIBXLS_ERROR_READ;
        goto cleanup;
    ...
    switch (tmp.id) {
      ...
      case XLS_RECORD_MULRK:
        ...
        if (pWS->rows.lastcol <
            xlsShortVal(((MULRK*)buf)->col) + (tmp.size - 6) / 6 - 1)
          pWS->rows.lastcol =
              xlsShortVal(((MULRK*)buf)->col) + (tmp.size - 6) / 6 - 1;
      ...
      case XLS_RECORD_MULBLANK:
        ...
        if (pWS->rows.lastcol <
            xlsShortVal(((MULBLANK*)buf)->col) + (tmp.size - 6) / 2 - 1)
          pWS->rows.lastcol =
              xlsShortVal(((MULBLANK*)buf)->col) + (tmp.size - 6) / 2 - 1;
...
xls_error_t xls_parseWorkSheet(xlsWorkSheet* pWS) {
  ...
  // printf("size=%d fatpos=%d)\n", pWS->workbook->olestr->size,
  // pWS->workbook->olestr->fatpos);
  ...
  do {
    ...
    if (xls_debug > 10) {
      printf("LASTPOS=%ld pos=%d filePos=%d filePos=%d\n", lastPos,
             (int)pWB->olestr->pos, pWS->filepos, pWB->filepos);
    }
    if ((read = ole2_read(&tmp, 1, 4, pWS->workbook->olestr)) != 4) {
      if (xls_debug) fprintf(stderr, "Error: failed to read OLE size\n");
      retval = LIBXLS_ERROR_READ;
    ...
    switch (tmp.id) {
      ...
      case XLS_RECORD_STRING:
        if (cell && (cell->id == XLS_RECORD_FORMULA ||
          ...
          xls_cell_set_str(
              cell, get_string((char*)buf, tmp.size, (BYTE)!pWB->is5ver, pWB));
          if (xls_debug) xls_showCell(cell);
        }
        break;
...
static xlsWorkBook* xls_open_ole(OLE2* ole, const char* charset,
  ...
  // open Workbook
  if (!(pWB->olestr = ole2_fopen(ole, "Workbook")) &&
      !(pWB->olestr = ole2_fopen(ole, "Book"))) {
...
cleanup:
  if (retval != LIBXLS_OK) {
    if (!pWB->olestr) ole2_close(ole);
    xls_close_WB(pWB);
    pWB = NULL;
  }
  if (outError) *outError = retval;
...
xlsRow* xls_row(xlsWorkSheet* pWS, WORD cellRow) {
  ...
  if (pWS->rows.row == NULL) return NULL;
...
xlsCell* xls_cell(xlsWorkSheet* pWS, WORD cellRow, WORD cellCol) {
  ...
  if ((row = xls_row(pWS, cellRow)) == NULL) return NULL;
...
#ifdef HAVE_ICONV
  if (pWB->converter) iconv_close((iconv_t)pWB->converter);
  if (pWB->utf16_converter) iconv_close((iconv_t)pWB->utf16_converter);
#endif

  if (pWB->utf8_locale) xls_freelocale((xls_locale_t)pWB->utf8_locale);
...
const char* xls_getVersion(void) { return VERSION; }

const char* xls_getError(xls_error_t code) {
  if (code == LIBXLS_OK) return "No error";
  if (code == LIBXLS_ERROR_READ) return "Unable to read from file";
  if (code == LIBXLS_ERROR_OPEN) return "Unable to open file";
  if (code == LIBXLS_ERROR_SEEK) return "Unable to seek within file";
  if (code == LIBXLS_ERROR_MALLOC) return "Unable to allocate memory";
  if (code == LIBXLS_ERROR_PARSE) return "Unable to parse file";
...
// or google "DocumentSummaryInformation and UserDefined Property Sets" and look
// for MSDN hits
//
...
static void xls_dumpSummary(char* buf, int isSummary, xlsSummaryInfo* pSI) {
  ...
  // head->format[2], head->format[3]); printf("  count=%x\n", head->count);

  for (i = 0; i < head->count; ++i) {
    secList = &head->secList[i];
    // printf("Section %d:\n", i);
    // printf("  class=%8.8x%8.8x%8.8x%8.8x\n", secList->format[0],
    // secList->format[1], secList->format[2], secList->format[3]); printf("
    // offset=%d (now at %ld\n", secList->offset, (char *)secList - (char *)buf
    // + sizeof(sectionList));
