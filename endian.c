...
void xlsConvertFormat(FORMAT *f) { W_ENDIAN(f->index); }

void xlsConvertBoundsheet(BOUNDSHEET *b) { D_ENDIAN(b->filepos); }
...
void xlsConvertPss(PSS *pss) {
  ...
  for (i = 0; i < 8; i++) pss->guid[i] = xlsShortVal(pss->guid[i]);
